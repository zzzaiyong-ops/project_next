...
function exportS7ExcelFiltered(camps){
  ...
  var hdrs = ['입금예정일','캠페인코드(KEY)','캠페인명','상품코드','MCN업체','인플루언서','당사유입수','구매자수','구매전환율(%)','신규회원수','공구가(원)','순주문수','순주문 금액(정산기준)','광고수수료(%)','수수료 광고비(부가세포함)','수수료 광고비2(부가세별도)','정액 광고비(부가세별도)','별도 광고비(메타광고)','세금계산서 공급가액(부가세별도)','릴스조회수','릴스댓글','정산일자','이슈 및 결과','캠페인 시작일','캠페인 종료일'];
  var ws = {};
  ws['!cols'] = [{wch:14},{wch:14},{wch:22},{wch:14},{wch:14},{wch:14},{wch:11},{wch:10},{wch:13},{wch:10},{wch:13},{wch:11},{wch:18},{wch:16},{wch:20},{wch:20},{wch:18},{wch:18},{wch:22},{wch:11},{wch:10},{wch:14},{wch:24},{wch:14},{wch:14}];
  var R = 0;
  ...
  var _guides = ['조회용(입력불필요)','','조회용(입력불필요)','상품코드 입력','MCN업체명','인플루언서명','건수 입력','건수 입력','자동계산(입력불필요)','건수 입력','원 단위 숫자\n(예: 59000)','건수 입력','자동계산(입력불필요)','% 숫자 입력\n(예: 20 → 20%)\n(0.2입력시 자동변환)','자동계산(입력불필요)','자동계산(입력불필요)','원 단위 직접 입력','원 단위 직접 입력','자동계산(입력불필요)','만 단위 숫자','건수 입력','날짜 형식\n(예: 2026-05-01)','텍스트 입력','조회용(입력불필요)','조회용(입력불필요)'];
