...
window.addEventListener('load', function(){ _suppressListener = false; _myLastSaveTime = 0; _myLastSaveCamps = null; });
...
function mergeRemote(remote){
  var merged = {
    ...
    campaigns:    remote.campaigns    ? Object.values(remote.campaigns).map(function(camp){
...
function _demoSampleCampaigns(){
  ...
  return [
    ...
    {id:1003,name:'신세계 맨즈컬렉션',stage:'2.캠페인확정',cat:'패션',role:'신규',infSize:'메가',revenue:80000000,start:add(7),end:add(14),owner:'박민준',pdSingle:'박민준',mcn:'패션MCN',infName:'멘즈패션C',campType:'방송',mdcat:'패션',infData:[{infName:'멘즈패션C',infSize:'메가',feeRate:18}]},
    ...
    {id:1010,name:'나이키 러닝화 신상',stage:'4.MCN요청',cat:'패션',role:'신규',infSize:'미들',revenue:45000000,start:add(8),end:add(15),owner:'박민준',pdSingle:'박민준',mcn:'스타MCN',infName:'스포츠H',infData:[{infName:'스포츠H',infSize:'미들',feeRate:14}]},
...
// NAV
// ═══════════════════════════════════════
const PAGE_TITLES={dashboard:'대시보드',s1:'1단계 · 캠페인요청',s2:'2단계 · 캠페인확정',s3:'3단계 · 상품정보등록',s4:'4단계 · MCN요청',s5:'5단계 · 인플루언서확정',s6:'6단계 · APP마케팅',s7:'7단계 · 정산',s8:'8단계 · 성과분석',influencers:'인플루언서 DB',campaigns:'전체 캠페인',usermgmt:'사용자 관리',mcnmgmt:'MCN 업체 관리',basemgmt:'기본정보 관리'};
...
function _goPageInternal(page, el, scrollTop){
  ...
  document.getElementById('page-ttl').textContent = PAGE_TITLES[page]||page;
...
function renderCalendar(){
  ...
  monthCamps.forEach(function(c){
    ...
    if(campSlot[c.id]===undefined) campSlot[c.id] = MAX_ROWS;
  });
  ...
  for(var d=1;d<=daysInMonth;d++){
    ...
    monthCamps.forEach(function(c){
      ...
      var infSize = prod ? prod.infSize : (c.infSize||'');
    ...
    var barsHtml = '';
    for(var row2=0;row2<MAX_ROWS;row2++){
      if(slotBars[row2]) barsHtml += slotBars[row2];
      else barsHtml += '<div style="height:16px;margin-bottom:0"></div>';
    }
    if(overflowCount>0){
      barsHtml += '<div style="font-size:9px;color:var(--text3);padding:1px 2px">+'+overflowCount+'개</div>';
    }
  ...
  var doneRevStr = doneRev > 0 ? (doneRev/100000000).toFixed(1)+'억원' : '-';
  var sumEl = document.getElementById('cal-summary');
  if(sumEl){
    sumEl.innerHTML = '<span style="font-size:12px;font-weight:700;color:var(--text2)">진행완료 캠페인</span>'
      ...
      + (doneCamps.length < doneAll.length ? '<span style="font-size:11px;color:var(--text3);margin-left:6px">※ 매출 입력된 캠페인 기준</span>' : '');
  }
}
...
function renderDash(){
  ...
  function campInMonth(c, yr, mo){
    // start/startDate 둘 다 체크
    var cStart = c.start || c.startDate || null;
    var cEnd   = c.end   || c.endDate   || null;
    // 날짜 없으면 → 등록 월에 표시 (현재 월 포함)
    if(!cStart && !cEnd){
      var now3=new Date(); return now3.getFullYear()===yr&&now3.getMonth()===mo;
    }
    var s=cStart?new Date(cStart):null;
    var e=cEnd?new Date(cEnd):null;
    // Invalid Date 방어
    if(s&&isNaN(s.getTime())) s=null;
    if(e&&isNaN(e.getTime())) e=null;
    if(!s&&!e){ var now4=new Date(); return now4.getFullYear()===yr&&now4.getMonth()===mo; }
    var mS=new Date(yr,mo,1), mE=new Date(yr,mo+1,0);
    if(s&&e) return s<=mE&&e>=mS;
    if(s) return s.getFullYear()===yr&&s.getMonth()===mo;
    if(e) return e.getFullYear()===yr&&e.getMonth()===mo;
    return false;
  }
  ...
  var ROLES = [
    ...
    {key:'신규',  color:'var(--blue)',   bg:'var(--blue-bg)'},
  ...
  ROLES.forEach(function(r){
    var campsByMonth = months.map(function(m){
      return DB.campaigns.filter(function(c){
        ...
        return rm&&campInMonth(c,m.year,m.month);
      });
    });
...
function renderS1(filter=''){
  ...
  _pg_s1.items.forEach(function(c, _pi){
    ...
    var roleColor = c.role==='메가'?'var(--pink)':c.role==='앵콜'?'var(--orange)':c.role==='신규'?'var(--blue)':c.role==='시딩'?'var(--green)':'var(--text3)';
    var roleBg   = c.role==='메가'?'var(--pink-bg)':c.role==='앵콜'?'var(--orange-bg)':c.role==='신규'?'var(--blue-bg)':c.role==='시딩'?'var(--green-bg)':'var(--bg4)';
    // 예상매출 억단위 변환 (소수점 1자리)
    var revStr = '-';
    if(c.revenue){
      var uck = c.revenue / 100000000;
      if(uck >= 0.1){
        revStr = uck.toFixed(1) + '억';
      } else {
        revStr = (c.revenue / 10000).toFixed(0) + '만';
      }
    }
    // MDCAT: 상품코드에서 가져오거나 직접 입력값
    ...
    rows+=`<tr onclick="openCampReqDetail(${c.id})" style="cursor:pointer">
      ...
      <td style="color:var(--text3);font-size:12px">${(c.start||c.startDate||'-').slice(5)}</td>
      <td style="color:var(--text3);font-size:12px">${(c.end||c.endDate||'-').slice(5)}</td>
...
function renderS4(){
  ...
  _pg_s4.items.forEach(function(c, _pi){
    ...
    rows += '<tr onclick="editProd('+c.id+')" style="cursor:pointer">'+'<td style="text-align:center;font-size:11px;font-weight:700;color:var(--text3);width:36px">'+rowNum+'</td>'
      ...
      +'<td style="color:var(--text3);font-size:12px;white-space:nowrap">'+((c.start||c.startDate||'-').slice(5))+'</td>'
      +'<td style="color:var(--text3);font-size:12px;white-space:nowrap">'+((c.end||c.endDate||'-').slice(5))+'</td>'
      +'<td><div class="row-acts" style="opacity:1">'
...
function openAppMktEdit(campId){
  ...
  liveDiv.innerHTML = '<label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px">모바일라이브 방송일시 <span style="color:var(--red)">*</span></label>'
    + '<input class="inp" id="modal-live-dt" type="datetime-local" value="'+(appMkt.liveDt||'')+'">';
...
function saveAppMktEdit(campId){
  ...
  if(idx===-1) return;
  DB.campaigns[idx] = Object.assign({}, DB.campaigns[idx], {
    appMkt: {channels:channels, superStart:superStart, superEnd:superEnd, liveDt:liveDt},
...
function toggleAppMktNa(){
  ...
  var superFlds = document.getElementById('appmkt-super-fields');
  var liveFlds  = document.getElementById('appmkt-live-fields');
  if(na){
    // 해당없음 체크 → 모든 채널 체크 해제 + 영역 흐리게
    ...
    if(superFlds) superFlds.style.display='none';
    if(liveFlds)  liveFlds.style.display='none';
  } else {
...
function toggleAppMktFields(){
  ...
  var sf = document.getElementById('appmkt-super-fields');
  var lf = document.getElementById('appmkt-live-fields');
  if(sf) sf.style.display = superChecked?'block':'none';
  if(lf) lf.style.display = liveChecked?'block':'none';
}
...
function renderS7(){
  ...
  var validCampIds = DB.campaigns.filter(function(c){
    // target 모드에서는 날짜 필터 무시 (처리 대상은 기간 상관없이 표시)
  ...
  _pg_s7.items.forEach(function(camp, _pi7){
    ...
    infList.forEach(function(inf, infIdx){
      ...
      if(infIdx === 0){
        // 첫 행: 캠페인명/날짜/MDCAT/체크박스 rowspan
      ...
      rows += '<td style="font-size:12px">'+(infSize!=='-'?'<span style="background:var(--bg3);padding:1px 6px;border-radius:10px;font-size:11px">'+infSize+'</span>':'-')+'</td>'
        ...
        +'<td style="font-weight:700;color:var(--orange)">'+costStr+'</td>';
...
function renderCamps(){
  ...
  var filtered = DB.campaigns.filter(function(c){
    if(!c.name || !c.name.trim()) return false; // 이름 없는 빈 캠페인 제외
    // MD 권한: 본인 담당 캠페인만 (isMycamp과 동일 기준)
  ...
  paged.forEach(function(c, i){
    ...
    rows += '<tr onclick="editProd('+c.id+')">'
      ...
      + '<td><div class="row-acts">'
      + '<button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();editProd('+c.id+')">✏️ 수정</button>'
...
function clearCampFilter(){
  ['sf-camp-stage','sf-camp-owner','sf-camp-md','sf-camp-mcn','sf-camp-name','df-camp-from','df-camp-to'].forEach(function(id){
...
function _delCampConfirmed(id){
  ...
  renderS1(); renderS2(); renderCamps(); renderDash(); updateBadges(); closeDetail(); showToast('삭제됨');
  if(fbReady){
    // 리스너가 삭제 전 데이터를 복원하지 않도록 충분히 억제
    _suppressListener = true;
    fbDB.ref('influencer-hub/campaigns/' + sid).remove()
      .then(function(){
        console.log('[delCamp] 삭제 성공:', sid);
        // 삭제 완료 후 전체 DB를 Firebase에 다시 씀 (삭제 반영 확정)
        return pushToFirebase();
      })
      .then(function(){
        setTimeout(function(){ _suppressListener = false; }, 3000);
      })
      .catch(function(e){
        console.error('[delCamp] 삭제 실패:', e.code, e.message);
...
function renderInfs(){
  ...
  // ── 테이블 헤더 ──
  var tableHtml = '<div class="card"><table style="width:100%"><thead><tr>'    +'<th>인플루언서</th><th>최근 규모</th><th>최근 카테고리</th>'    +'<th style="text-align:right">평균매출</th><th style="text-align:right">평균주문건수</th><th style="text-align:right">평균비용</th>'    +'</tr></thead><tbody>';
...
function openInfDetail(infName){
  ...
  var perfRows = perfCamps.map(function(camp){
    var rev = camp.settleRevenue||0;
    var revStr = rev>=100000000?(rev/100000000).toFixed(1)+'억':rev>=10000?(rev/10000).toFixed(0)+'만':rev>0?rev.toLocaleString():'-';
    var fr=parseFloat(camp.feeRate)||0, fa=parseInt(camp.feeAmount)||0;
    var ar=parseFloat(camp.agencyRate)||0, da=parseInt(camp.settleDa)||0;
    var cost=Math.round(rev*fr/100)+fa+Math.round(rev*ar/100)+da;
    var costStr=cost>=10000?(cost/10000).toFixed(0)+'만':cost>0?cost.toLocaleString():'-';
    var views=camp.s7Perf?(camp.s7Perf.views||0)+'만':'-';
    return '<tr style="font-size:12px">'      +'<td style="font-weight:600;padding:6px 8px">'+camp.name+'</td>'      +'<td style="color:var(--text3);padding:6px 8px">'+(camp.brand||(camp.skus&&camp.skus[0]?camp.skus[0].brand:'')||'-')+'</td>'      +'<td style="color:var(--text3);padding:6px 8px">'+(camp.cat||'-')+'</td>'      +'<td style="color:var(--green);font-weight:700;text-align:right;padding:6px 8px">'+revStr+'</td>'      +'<td style="color:var(--text2);text-align:right;padding:6px 8px">'+(camp.settleOrders||'-')+'</td>'      +'<td style="color:var(--text3);text-align:right;padding:6px 8px">'+views+'</td>'      +'<td style="color:var(--orange);text-align:right;padding:6px 8px">'+costStr+'</td>'      +'</tr>';
  }).join('');
  ...
  var sizeBg    = size.includes('메가')?'var(--pink-bg)':size.includes('미들')?'var(--blue-bg)':size.includes('시딩')?'var(--green-bg)':'var(--bg4)';
  var platHtml  = '';
  if(ytCh) platHtml += '<a href="https://www.youtube.com/@'+ytCh.replace('@','')+'" target="_blank" style="display:inline-flex;align-items:center;gap:3px;background:#ff000022;color:#ff0000;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;text-decoration:none;margin-right:6px">▶ 유튜브 '+ytCh+'</a>';
  if(igCh) platHtml += '<a href="https://www.instagram.com/'+igCh.replace('@','')+'" target="_blank" style="display:inline-flex;align-items:center;gap:3px;background:#e1306c22;color:#e1306c;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;text-decoration:none">📸 인스타 '+igCh+'</a>';
  if(twCh) platHtml += '<a href="https://x.com/'+twCh.replace('@','')+'" target="_blank" style="display:inline-flex;align-items:center;gap:3px;background:#00000015;color:#000;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;text-decoration:none">𝕏 '+twCh+'</a>';

  el.innerHTML = '<div style="background:var(--bg2);border:1px solid var(--border2);border-radius:var(--r-lg);padding:0;min-width:620px;max-width:800px;width:95%;max-height:85vh;display:flex;flex-direction:column">'    +'<div style="padding:20px 24px 16px;border-bottom:1px solid var(--border2);display:flex;align-items:center;justify-content:space-between">'    +'<div style="display:flex;align-items:center;gap:12px">'    +'<div class="iav" style="width:40px;height:40px;font-size:16px;background:'+avColor(inf.name)+'22;color:'+avColor(inf.name)+'">'+inf.name[0]+'</div>'    +'<div><div style="font-size:17px;font-weight:800">'+inf.name+'</div>'    +'<div style="margin-top:4px;display:flex;align-items:center;gap:8px">'    +'<span style="background:'+sizeBg+';color:'+sizeColor+';padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700">'+size+'</span>'    +(platHtml||'')+'</div></div></div>'    +'<button class="btn btn-ghost btn-xs" onclick="document.getElementById(&quot;inf-detail-modal&quot;).remove()">✕</button>'    +'</div>'    +'<div style="padding:14px 24px;display:grid;grid-template-columns:repeat(3,1fr);gap:10px;border-bottom:1px solid var(--border2)">'    +'<div style="background:var(--bg3);border-radius:var(--r-sm);padding:10px;text-align:center"><div style="font-size:10.5px;color:var(--text3);margin-bottom:4px">수수료율</div><div style="font-weight:700;color:var(--accent2);font-size:14px">'+(lc.feeRate?lc.feeRate+'%':'-')+'</div></div>'    +'<div style="background:var(--bg3);border-radius:var(--r-sm);padding:10px;text-align:center"><div style="font-size:10.5px;color:var(--text3);margin-bottom:4px">원고료</div><div style="font-weight:700;color:var(--accent2);font-size:14px">'+(lc.feeAmount?(lc.feeAmount/10000).toFixed(0)+'만원':'-')+'</div></div>'    +'<div style="background:var(--bg3);border-radius:var(--r-sm);padding:10px;text-align:center"><div style="font-size:10.5px;color:var(--text3);margin-bottom:4px">카테고리</div><div style="font-weight:700;color:var(--text);font-size:14px">'+(lc.cat||'-')+'</div></div>'    +'</div>'    +'<div style="padding:14px 24px 0;font-size:13px;font-weight:700;color:var(--text2)">📋 캠페인 실적 (최근 2년)</div>'    +'<div style="overflow-y:auto;padding:0 24px 20px">'    +(perfCamps.length      ?'<table style="width:100%;margin-top:10px"><thead><tr style="background:var(--bg3)">'        +'<th style="padding:6px 8px;text-align:left;font-size:11.5px">캠페인명</th><th style="padding:6px 8px;text-align:left;font-size:11.5px">브랜드</th>'        +'<th style="padding:6px 8px;text-align:left;font-size:11.5px">카테고리</th><th style="padding:6px 8px;text-align:right;font-size:11.5px">매출</th>'        +'<th style="padding:6px 8px;text-align:right;font-size:11.5px">주문건수</th><th style="padding:6px 8px;text-align:right;font-size:11.5px">조회수</th>'        +'<th style="padding:6px 8px;text-align:right;font-size:11.5px">비용</th></tr></thead><tbody>'+perfRows+'</tbody></table>'      :'<div style="padding:24px;text-align:center;color:var(--text3)">진행 실적 없음</div>')    +'</div></div>';
...
function openCopyCampModal(){
  ...
  el.innerHTML = '<div style="background:var(--bg2);border:1px solid var(--border2);border-radius:var(--r-lg);padding:0;min-width:560px;max-width:700px;width:100%;max-height:80vh;display:flex;flex-direction:column">'
    ...
    + '<div style="font-size:16px;font-weight:800">📋 캠페인 불러오기</div>'
    ...
    + '<input class="inp" id="copy-camp-search" placeholder="캠페인명 검색..." oninput="filterCopyCamps(this.value)" style="width:100%">'
    + '</div>'
...
function renderCopyCampList(keyword){
  var list = DB.campaigns.filter(function(camp){
    return !keyword || camp.name.includes(keyword);
  }).sort(function(a,b){ return (b.start||'') > (a.start||'') ? 1 : -1; });
  ...
  el.innerHTML = list.map(function(camp){
    var roleColor = camp.role==='메가'?'var(--pink)':camp.role==='앵콜'?'var(--orange)':camp.role==='신규'?'var(--blue)':'var(--green)';
    var roleBg    = camp.role==='메가'?'var(--pink-bg)':camp.role==='앵콜'?'var(--orange-bg)':camp.role==='신규'?'var(--blue-bg)':'var(--green-bg)';
    ...
    return '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 24px;border-bottom:1px solid var(--border2);cursor:pointer;transition:background .15s" '
      ...
      + 'onclick="copyCampData('+camp.id+')">'
      + '<div style="flex:1;min-width:0">'
      + '<div style="font-weight:700;font-size:13.5px;margin-bottom:4px">'+camp.name+'</div>'
      ...
      + '</div>'
      + '</div>'
      + '<button class="btn btn-primary btn-sm" onclick="event.stopPropagation();copyCampData('+camp.id+')" style="margin-left:16px;white-space:nowrap">이 캠페인으로 복사</button>'
      + '</div>';
  }).join('');
}
...
function copyCampData(campId){
  var p = DB.campaigns.find(function(x){return x.id===campId;});
  if(!p) return;

  // 기존 등록 모달 초기화
  document.getElementById('p-edit-id').value = '';
  document.getElementById('prod-mo-title').textContent = '캠페인 등록 (1단계 · 캠페인 요청)';
  ...
  showToast('캠페인 정보를 불러왔습니다. 캠페인명과 날짜를 입력해주세요.');
}
...
function clearS7Filter(){
  var df = document.getElementById('s7-date-from'); if(df) df.value='';
  var dt = document.getElementById('s7-date-to');   if(dt) dt.value='';
  renderReports();
}
...
function renderReports(){
  ...
  var camps = DB.campaigns.filter(function(camp){
    var startD = camp.start ? new Date(camp.start) : null;
    var endD   = camp.end   ? new Date(camp.end)   : null;
    if(!startD) return false;
    if(startD > today) return false; // 아직 시작 안 한 캠페인 제외

    // 진행중/종료 필터
    if(s7Filter === 'active'){
      if(endD && endD < today) return false; // 종료된 캠페인 제외
    } else {
      if(!endD || endD >= today) return false; // 종료 안 된 캠페인 제외
    }
  ...
  camps.forEach(function(camp){
    ...
    rows += '<tr style="cursor:pointer" onclick="editProd('+c.id+')">' 
      ...
      + '<td><button class="btn btn-ghost btn-xs" onclick="openS7Detail('+camp.id+')">상세</button></td>'
      + '</tr>';
  });

  document.getElementById('rep-tbl').innerHTML = rows ||
    '<tr><td colspan="11" class="empty" style="padding:32px;text-align:center;color:var(--text3)">해당 조건의 캠페인이 없습니다</td></tr>';
}
...
function openS7Detail(campId){
  ...
  el.innerHTML = '<div style="background:var(--bg2);border:1px solid var(--border2);border-radius:var(--r-lg);padding:28px 32px;min-width:480px;max-width:600px;width:100%;max-height:90vh;overflow-y:auto">'
    ...
    + '<div style="display:flex;justify-content:space-between"><span style="color:var(--text3)">DA 광고료</span><span style="font-weight:600">'+fmt(daFee)+'</span></div>'
...
function exportExcel(type){
  ...
  if(type==='products'){
    ...
    ws=XLSX.utils.json_to_sheet((DB.settlements||[]).map(s=>({캠페인:getCamp(s.campaign).name,인플루언서:getInf(s.inf).name,전체매출:s.revenue,인플루언서수수료:s.infFee,에이전시수수료:s.agencyFee,DA광고비:s.daFee,정산상태:s.status})));
    name='정산';
  } else if(type==='report'){
    ws=XLSX.utils.json_to_sheet(DB.influencers.map(i=>({이름:i.name,채널:i.handle,플랫폼:i.platform,팔로워:i.followers,참여율:i.engage,카테고리:i.cat,단가:i.fee,협업횟수:DB.matches.filter(m=>m.inf===i.id&&m.status==='승인').length})));
    name='성과리포트';
  }
...
function openNewProd(){
  ...
  var suf2=document.getElementById('appmkt-super-fields'); if(suf2) suf2.style.display='none';
  var luf2=document.getElementById('appmkt-live-fields');  if(luf2) luf2.style.display='none';
  ['appmkt-super-start','appmkt-super-end','appmkt-live-dt','appmkt-landing-url','appmkt-deal-url'].forEach(function(id){ var el=document.getElementById(id); if(el) el.value=''; });
...
function saveProd(){
  ...
  if(!budget.trim())     missing.push('예산');
  if(!role)              missing.push('캠페인 역할');
  ...
  var liveChecked  = document.getElementById('appmkt-live')?.checked;
  if(superChecked){
    if(!document.getElementById('appmkt-super-start')?.value) missing.push('[APP마케팅] 슈퍼브랜드 시작일');
    if(!document.getElementById('appmkt-super-end')?.value)   missing.push('[APP마케팅] 슈퍼브랜드 종료일');
  }
  if(liveChecked && !document.getElementById('appmkt-live-dt')?.value) missing.push('[APP마케팅] 모바일라이브 방송일시');
  ...
  var skus = [...skuRows2].map(function(row, i){
    ...
    return {
      ...
      finalPrice: md>0 ? Math.max(0,md-card-mile-cpn) : 0,
    };
  }).filter(function(s){
  ...
  var newData = {
    ...
    owner:v('p-owner'), revenue:iv('p-revenue'),
    ...
    mdcat:v('p-mdcat'),
    campType: (document.getElementById('p-camptype-online')?.checked ? '온라인' :
               document.getElementById('p-camptype-broadcast')?.checked ? '방송' : ''),
...
function addInfBlock(data){
  ...
  block.innerHTML =
    ...
    +'<input class="inp inf-name-'+n+'" placeholder="이름 또는 채널명" value="'+(d.infName||'')+'"></div>'
    +'</div>'
    // 채널
    ...
    +'<input class="inp inf-frate-'+n+'" type="number" placeholder="15" step="0.1" value="'+(d.feeRate||'')+'"></div>'
    ...
    +'<input class="inp inf-saddr-'+n+'" placeholder="수신자명 / 연락처 / 주소" value="'+(d.sampleAddress||'')+'"></div>'
    +'</div>';
...
function addSettleBlock(n, data){
  var container = document.getElementById('settle-blocks-container');
  if(!container) return;
  var d = data || {};
  ...
  block.innerHTML =
    '<div class="settle-inf-label" style="font-size:12.5px;font-weight:700;color:var(--accent2);margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border2)">💰 정산 정보 - 인플루언서 '+n+'</div>'
    // ── 매출/주문
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">'
    +'<div><label style="font-size:11.5px;font-weight:600;margin-bottom:4px;display:block">전체 매출 (원)</label>'
    +'<input class="inp inp-money settle-rev-'+n+'" placeholder="0" value="'+(d.revenue?d.revenue.toLocaleString('ko-KR'):'')+'"></div>'
    +'<div><label style="font-size:11.5px;font-weight:600;margin-bottom:4px;display:block">주문건수</label>'
    +'<input class="inp settle-ord-'+n+'" type="number" placeholder="0" value="'+(d.orders||'')+'"></div>'
    +'</div>'
    // ── 비용 계산 (인플루언서별)
    +'<div style="background:var(--bg3);border:1px solid var(--border2);border-radius:var(--r-sm);padding:12px;font-size:12px">'
    +'<div style="font-weight:700;color:var(--text2);margin-bottom:8px;font-size:12px">📊 비용 계산</div>'
    +'<div style="display:grid;grid-template-columns:1fr auto;gap:6px 12px;align-items:center">'
    // 정률 수수료
    +'<div style="color:var(--text3)">수수료율 매출수수료 <span class="settle-frate-disp-'+n+'" style="color:var(--accent);font-size:10.5px;font-weight:600"></span></div>'
    +'<div style="text-align:right;font-weight:600" class="settle-calc-fr-'+n+'">-</div>'
    // 정액 원고료
    +'<div style="color:var(--text3)">+ 원고료 (정액) <span class="settle-famt-disp-'+n+'" style="color:var(--accent);font-size:10.5px;font-weight:600"></span></div>'
    +'<div style="text-align:right;font-weight:600" class="settle-calc-fa-'+n+'">-</div>'
    // 에이전시
    +'<div style="color:var(--text3)">+ 에이전시 수수료 (cafe24) <span class="settle-arate-disp-'+n+'" style="color:var(--accent);font-size:10.5px;font-weight:600"></span></div>'
    +'<div style="text-align:right;font-weight:600" class="settle-calc-ar-'+n+'">-</div>'
    // DA
    +'<div style="color:var(--text3)">+ DA 광고료</div>'
    +'<div style="text-align:right;font-weight:600" class="settle-calc-da-'+n+'">-</div>'
    // 소계
    +'<div style="font-weight:800;color:var(--text);padding-top:6px;border-top:1px solid var(--border2);margin-top:4px">비용 소계</div>'
    +'<div style="text-align:right;font-weight:800;font-size:14px;color:var(--accent2);padding-top:6px;border-top:1px solid var(--border2);margin-top:4px" class="settle-cost-'+n+'">-</div>'
    +'</div>'
    +'</div>';

  // money 포맷 + 자동 계산
  block.querySelectorAll('.inp-money').forEach(function(el){
    el.addEventListener('input', function(){ formatMoney(this); updateSettleCostDisplay(block, n); });
  });
  block.querySelectorAll('input[type="number"]').forEach(function(el){
    el.addEventListener('input', function(){ updateSettleCostDisplay(block, n); });
  });
  container.appendChild(block);
  updateSettleCostDisplay(block, n);
}
...
function updateSettleCostDisplay(block, n){
  ...
  var rev  = parseInt((block.querySelector('.settle-rev-'+n)?.value||'').replace(/,/g,''))||0;
  var fr=0, fa=0, ar=0;
  if(infBlock){
...
function getSettleBlocksData(){
  ...
  document.querySelectorAll('.settle-block').forEach(function(block, idx){
    var n = idx+1;
    result.push({
      revenue: parseInt((block.querySelector('.settle-rev-'+n)?.value||'').replace(/,/g,''))||0,
      orders:  parseInt(block.querySelector('.settle-ord-'+n)?.value)||0,
    });
  });
  return result;
}

function renderSettleBlocks(settleList){
  var container = document.getElementById('settle-blocks-container');
  if(!container) return;
  container.innerHTML = '';
  if(settleList && settleList.length > 0){
    settleList.forEach(function(d,i){ addSettleBlock(i+1, d); });
  }
}
...
function saveInf(){
  ...
  DB.influencers.push({id:nid.influencers++,name,handle:v('i-handle'),platform:v('i-plat'),followers:iv('i-flw'),engage:parseFloat(v('i-eng'))||0,cat:v('i-cat'),contact:v('i-contact'),fee:iv('i-fee'),memo:v('i-memo')});
...
function updateBadges(){
  ...
  // S2: 2단계 확정 대상 = 1단계 캠페인요청
  var s2cnt = camps.filter(function(c){ return c.stage==='1.캠페인요청'; }).length;
...
function _searchSelUsers(role){
  return role === 'md' ? MD_USERS : MANAGER_USERS;
}
...
function switchBmTab(tab, el){
  ...
  ['mdcat','users','roles','mcn'].forEach(function(t){
...
function downloadCampTemplate(){
  ...
  var sample  = ['2026 여름 선케어','2026-06-01','2026-07-15','150000000','5000000','신규','홍길동','김민지','선케어랩','(주)선케어랩','뷰티','BEAUTY-01','(주)크리에이터스','SKU-001','UV선크림50ml','89000','5000','3000','2000','워터프루프 SPF50+','브랜드력,가격경쟁력'];
  ...
  reader.onload = function(e){
    try {
      ...
      _campBulkData = data.map(function(r){
        ...
        if(!['메가','앵콜','신규','시딩'].includes(role)) err.push('역할 오류('+role+')');
