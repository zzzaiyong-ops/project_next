...
function renderDash(){
  ...
  var totalCounts   = months.map(function(m){return DB.campaigns.filter(function(c){return (c.campType||'')!=='모바일라이브'&&campInMonth(c,m.year,m.month);}).length;});
  var totalRevenues = months.map(function(m){return DB.campaigns.reduce(function(sum,c){return sum+((c.campType||'')!=='모바일라이브'&&campInMonth(c,m.year,m.month)?(c.revenue||0):0);},0);});
  ...
  months.forEach(function(mo, idx){
    var allMo = DB.campaigns.filter(function(c){return (c.campType||'')!=='모바일라이브'&&campInMonth(c,mo.year,mo.month);});
  ...
  var roleRows = '<tr style="background:var(--accent-bg)">'
    ...
    + months.map(function(mo){
        var onl = DB.campaigns.filter(function(c){return (c.campType||'')!=='모바일라이브'&&campInMonth(c,mo.year,mo.month)&&isOnline(c);});
        var brd = DB.campaigns.filter(function(c){return (c.campType||'')!=='모바일라이브'&&campInMonth(c,mo.year,mo.month)&&isBroadcast(c);});
...
function editProd(id){
  ...
  }], p.skus||[], (p.end||p.endDate||'').slice(0,10));
  // 쇼호스트 복원
  ...
  _pg_s7.items.forEach(function(camp, _pi7){
    ...
    if(_s7Mlive){
      ...
      infList.forEach(function(inf, infIdx){
        ...
        var payDate = (camp.settlePaymentDate||'').slice(0,10);
        var payDateDisp = payDate ? payDate.slice(5).replace('-','/') : '-'; // MM/DD
        var payColor = payDate ? (payDate.slice(8)==='30'?'var(--blue)':'var(--orange)') : 'var(--text3)';
        ...
        if(infIdx === 0){
          rows += '<td rowspan="'+rowspan+'" style="font-size:12px;font-weight:700;color:'+payColor+';white-space:nowrap;text-align:center">'+payDateDisp+'</td>'
            +'<td rowspan="'+rowspan+'">'+ statusBadge +'</td>'
...
function renderCamps(){
  ...
  // 정렬: 미확정(stage 1~4) 먼저 → 그 안에서 최신 등록(id 내림차순)
  // 확정(stage 5~) 이후도 최신 등록 순
  filtered.sort(function(a, b){
    var aId = (STAGES.find(function(s){ return s.name===a.stage; })||{id:9}).id;
    var bId = (STAGES.find(function(s){ return s.name===b.stage; })||{id:9}).id;
    var aConf = aId >= 5 ? 1 : 0; // 0=미확정, 1=확정
    var bConf = bId >= 5 ? 1 : 0;
    if(aConf !== bConf) return aConf - bConf; // 미확정 먼저
    return (b.id||0) - (a.id||0);             // 같은 그룹 내 최신 등록 먼저
  });
...
function uploadMliveExcel(input){
  ...
  reader.onload = function(e){
    ...
    // 헤더 행에서 컬럼 인덱스 자동 매핑
    // 데이터행 이전까지 모든 헤더행을 스캔 (다중 헤더행 지원, 최대 30행)
    // 예: Row1=대분류, Row2=소분류, Row3=세부항목(M라이브마케팅비 등)
    var colMap = {};
    // '마케팅' 키는 제외: row2에 단독 '마케팅'(idx=68) 섹션레이블이 exact-match로 먼저 걸려
    // headerKeywords['마케팅']=68이 되고, 이후 row3의 'M라이브마케팅비'(idx=87)를 건너뛰는 버그 방지
    var headerKeywords = {
      '순주문수량':null, '순주문금액':null, '주문이익율':null, '주문이익액':null,
      '총시청자수':null, '프로그램명':null, '방송구분':null, '방송일자':null,
      '대표담당MD':null, '시작':null, '종료':null, '광고수익':null
    };
    var mktFeeColIdx = null;  // 'M라이브마케팅비' 컬럼 — includes('마케팅비')로만 탐지
    for(var hi=0; hi<Math.min(30,rows.length); hi++){
      var hrow = rows[hi];
      // 데이터행 도달 시 탐색 종료 (편성코드: 6자리+ 숫자로 시작)
      if(String(hrow[0]||'').trim().match(/^\d{6,}/)) break;
      for(var hj=0; hj<hrow.length; hj++){
        ...
        // 마케팅비: '마케팅비' 포함어만 허용 (단독 '마케팅' 섹션레이블 idx=68과 구분)
        if(mktFeeColIdx===null && hv.includes('마케팅비')) mktFeeColIdx=hj;
      }
    }
    console.log('[M라이브] 헤더 매핑:', JSON.stringify(headerKeywords), '마케팅비col:', mktFeeColIdx);
    ...
    var CI = {
      ...
      mktFee:mktFeeColIdx!=null?mktFeeColIdx:87,
...
function renderWeeklyView(){
  ...
  // M라이브 주차합계에서 매칭된 편성코드 제외 (이중집계 방지)
  // 단, M라이브 전용 뷰(s8Type==='mlive')에서는 인플 행이 없으므로 제외 안 함
  if(s8Type !== 'mlive'){
  ...
  rows.forEach(function(row){
    if(row.type==='inf'){
      ...
      var _sd0gt=(c.settleData&&c.settleData[0])||{};
      ...
      var cost2=_sd0gt.taxSupply||(Math.round((_sd0gt.commFeeVat||0)/1.1)+(_sd0gt.fixedFee||0)+(_sd0gt.metaFee||0))||(Math.round(sr*fr2/100)+fa2+Math.round(sr*ar2/100)+da2);
      gt.infCost+=cost2;
  ...
  rows.forEach(function(row){
    if(row.type==='inf'){
      ...
      // 비용 = 세금계산서 공급가액(부가세별도) 우선, 없으면 구성항목 합산
      var inf0 = (c.infData&&c.infData[0])||{};
      ...
      var cost = sd0.taxSupply || (Math.round((sd0.commFeeVat||0)/1.1) + (sd0.fixedFee||0) + (sd0.metaFee||0)) || (Math.round(sr*fr/100)+fa+Math.round(sr*ar/100)+daFee);
  ...
  rows.forEach(function(row){
    if(row.type==='inf'){
      ...
      var cost=sd0.taxSupply||(Math.round((sd0.commFeeVat||0)/1.1)+(sd0.fixedFee||0)+(sd0.metaFee||0))||(Math.round(sr*fr/100)+fa+Math.round(sr*ar/100)+da);
...
function renderReports(){
  ...
  camps.forEach(function(camp){
    ...
    // 비용 = 세금계산서 공급가액(부가세별도) — 저장값 우선, 없으면 commFeeVat 기준 재계산
    var totalCost= sd0.taxSupply || (Math.round((sd0.commFeeVat||0)/1.1) + (sd0.fixedFee||0) + (sd0.metaFee||0));
...
function openS7Detail(campId){
  ...
  var taxSupply= sd0.taxSupply || (_d2CF2 + (sd0.fixedFee||0) + (sd0.metaFee||0));
...
function addInfBlock(data){
  ...
  block.innerHTML =
    ...
    +'<input class="inp inp-money inf-famt-'+n+'" placeholder="500,000" value="'+(d.feeAmount?d.feeAmount.toLocaleString('ko-KR'):'')+'" oninput="formatMoney(this);syncInfFamtToSettle(this,'+n+')"></div>'
...
function addSettleBlock(n, data, campaignSkus, campEndDate){
  var container = document.getElementById('settle-blocks-container');
  if(!container) return;
  var d = data || {};
  // 정산일자 자동 계산: 저장된 값 없으면 캠페인 마감일 + 7일
  var _campEnd = ((campEndDate||document.getElementById('p-end')?.value||'').slice(0,10));
  var _autoSettleDate = '';
  if(!d.settleDate && _campEnd){
    try{ var _sad=new Date(_campEnd); _sad.setDate(_sad.getDate()+7); _autoSettleDate=_sad.toISOString().slice(0,10); }catch(e){}
  }
  var fmtV = function(v){ return v ? parseInt(v).toLocaleString('ko-KR') : ''; };

  // ── SKU 행 목록: 저장된 skuItems + 캠페인 상품정보 병합
  var savedItems = d.skuItems || [];
  // 구버전 호환: skuItems 없고 구버전 필드(revenue/orders 또는 netAmt/netOrders) 있으면 마이그레이션
  if(!savedItems.length && (d.skuCode || d.netOrders || d.netAmt || d.orders || d.revenue)){
    savedItems = [{skuCode:d.skuCode||'', netOrders:d.netOrders||d.orders||0, convRate:d.convRate||0,
  ...
  block.innerHTML =
    ...
    +'<input class="inp inp-sm settle-settledate-'+n+'" type="date" value="'+(d.settleDate||_autoSettleDate)+'"></div>'
    +'</div>'
    // ── 이슈 및 결과
  ...
  container.appendChild(block);
  // 초기 자동계산: settle-na는 HTML에서 이미 저장값으로 설정됐으므로 덮어쓰지 않고
  // 전환율만 계산한 뒤 수수료 합산 (구버전 revenue 데이터 보호)
  block.querySelectorAll('.settle-sku-item').forEach(function(row){ calcSettleConvRate(row); });
...
// 인플루언서 N의 확정원고료 변경 → 정산 블록 N의 정액광고비(부가세별도) 동기화
function syncInfFamtToSettle(el, n){
  var raw = el.value.replace(/[^0-9]/g,'');
  var amt = parseInt(raw,10)||0;
  var block = document.querySelector('.settle-block[data-settle-n="'+n+'"]');
  if(!block) return;
  var fixedEl = block.querySelector('.settle-fixedfee-'+n);
  if(!fixedEl) return;
  fixedEl.value = amt ? amt.toLocaleString('ko-KR') : '';
  calcSettleBlockFees(block);
}
...
function renderSettleBlocks(settleList, campaignSkus, campEndDate){
  var container = document.getElementById('settle-blocks-container');
  if(!container) return;
  container.innerHTML = '';
  if(settleList && settleList.length > 0){
    settleList.forEach(function(d,i){ addSettleBlock(i+1, d, campaignSkus||[], campEndDate||''); });
  }
}
...
function _buildAdRequestDoc(year, month, mcnData, camps){
  ...
  camps.forEach(function(c){
    ...
    var cs = (c.start||c.startDate||'').slice(0,10);
    var ce = (c.end||c.endDate||'').slice(0,10);
    var feeRate = parseFloat(inf.feeRate||c.feeRate)||0;
    ...
    rowsHtml += '<tr>'
      ...
      +'<td class="tc">'+(feeRate ? feeRate+'%' : '')+'</td>'
      +'<td class="tl"></td>'
      +'</tr>';
  });
  ...
  return '<div class="doc">'
    ...
    +'<table class="tb">'
    +'<tr><th style="width:13%">일정</th><th style="width:24%">상품명</th><th style="width:15%">인플루언서명</th><th style="width:18%">정액광고비(VAT별도)</th><th style="width:14%">수수료율</th><th style="width:16%">비고</th></tr>'
    +rowsHtml
    +'</table>'
    // ── 대행사 담당자
...
// ── 광고신청서 엑셀 다운로드 ──
function generateAdRequestsXlsx(){
  if(typeof XLSX === 'undefined'){ showToast('엑셀 라이브러리 로드 실패 - 잠시 후 다시 시도하세요.'); return; }
  var year  = parseInt(document.getElementById('ar-year').value);
  var month = parseInt(document.getElementById('ar-month').value);
  if(!year || !month || month<1 || month>12){ showToast('년월을 정확히 입력하세요.'); return; }
  ...
  var wb = XLSX.utils.book_new();
  Object.keys(mcnGroups).sort().forEach(function(mcnName, idx){
    var camps   = mcnGroups[mcnName];
    var mcnData = MCN_COMPANIES.find(function(m){ return m.name===mcnName; }) || {name:mcnName};
    var ws      = _buildAdRequestSheet(year, month, mcnData, camps);
    var sheetName = (mcnName||'MCN'+(idx+1)).replace(/[\\\/\[\]\*\?:]/g,'_').slice(0,31);
    XLSX.utils.book_append_sheet(wb, ws, sheetName);
  });

  XLSX.writeFile(wb, year+'년 '+month+'월_인플루언서캠페인광고신청서.xlsx');
  closeMo('adrequest');
}

function _buildAdRequestSheet(year, month, mcnData, camps){
  var fmtD = function(d){
    if(!d) return '';
    var p = d.slice(0,10).split('-');
    return parseInt(p[1])+'/'+parseInt(p[2]);
  ...
  var ssg = {
    name:'주식회사 신세계라이브쇼핑', addr:'서울 성동구 아차산로 17, 10층',
    bizno:'502-86-28395', ceo:'문 성 욱',
    person:'송현주P', email:'shj2@shinsegae.com', phone:'010-2272-7873'
  };
  ...
  var BDR_MED = {
    top:{style:'medium',color:{rgb:'888888'}}, bottom:{style:'medium',color:{rgb:'888888'}},
    left:{style:'medium',color:{rgb:'888888'}}, right:{style:'medium',color:{rgb:'888888'}}
  };
  function hdr(v){ return {v:v,t:'s',s:{font:{name:'맑은 고딕',sz:10,bold:true},fill:{patternType:'solid',fgColor:{rgb:'DDE3F0'}},border:BDR,alignment:{horizontal:'center',vertical:'center',wrapText:true}}}; }
  function lbl(v){ return {v:v,t:'s',s:{font:{name:'맑은 고딕',sz:10,bold:true},fill:{patternType:'solid',fgColor:{rgb:'EDF0F8'}},border:BDR,alignment:{horizontal:'center',vertical:'center'}}}; }
  function cel(v){ return {v:v||'',t:'s',s:{font:{name:'맑은 고딕',sz:10},fill:{patternType:'solid',fgColor:{rgb:'FFFFFF'}},border:BDR,alignment:{horizontal:'left',vertical:'center'}}}; }
  function cen(v){ return {v:v||'',t:'s',s:{font:{name:'맑은 고딕',sz:10},fill:{patternType:'solid',fgColor:{rgb:'FFFFFF'}},border:BDR,alignment:{horizontal:'center',vertical:'center'}}}; }
  function num(v){ return {v:v||0,t:'n',s:{font:{name:'맑은 고딕',sz:10},fill:{patternType:'solid',fgColor:{rgb:'FFFFFF'}},border:BDR,alignment:{horizontal:'right',vertical:'center'},numFmt:'#,##0'}}; }
  function pct(v){ return {v:(v||0)/100,t:'n',s:{font:{name:'맑은 고딕',sz:10},fill:{patternType:'solid',fgColor:{rgb:'FFFFFF'}},border:BDR,alignment:{horizontal:'center',vertical:'center'},numFmt:'0%'}}; }
  function blk(){ return {v:'',t:'s',s:{fill:{patternType:'solid',fgColor:{rgb:'FFFFFF'}},border:BDR}}; }
  function ttl(v){ return {v:v,t:'s',s:{font:{name:'맑은 고딕',sz:15,bold:true},border:BDR_MED,alignment:{horizontal:'center',vertical:'center',letterSpacing:4}}}; }

  var aoa = [], merges = [], rows = [];
  var r = 0;
  function push(arr, hpx){ aoa.push(arr); rows.push({hpx:hpx||18}); r++; }
  function M(r1,c1,r2,c2){ merges.push({s:{r:r1,c:c1},e:{r:r2,c:c2}}); }
  ...
  // ─ Row 1: 제목 ─
  push([ttl('캠 페 인  광 고  신 청 서'),blk(),blk(),blk(),blk(),blk()], 34);
  M(r-1,0,r-1,5);
  ...
  // ─ Row 3: 섹션 헤더 ─
  push([hdr('캠페인 정보'),blk(),blk(),blk(),blk(),blk()], 22);
  M(r-1,0,r-1,5);

  // ─ Row 4: 광고주명 / MCN업체명 ─
  push([lbl('광고주명'),cel(ssg.name),blk(),lbl('MCN업체명'),cel(mcnData.name||''),blk()], 20);
  M(r-1,1,r-1,2); M(r-1,4,r-1,5);

  // ─ Row 5: 담당자명 / E-mail ─
  push([lbl('담당자명'),cel(ssg.person),blk(),lbl('E-mail'),cel(ssg.email),blk()], 20);
  M(r-1,1,r-1,2); M(r-1,4,r-1,5);

  // ─ Row 6: 전화번호 / 캠페인기간 ─
  push([lbl('전화번호'),cel(ssg.phone),blk(),lbl('캠페인 기간'),cel(year+'년 '+month+'월'),blk()], 20);
  M(r-1,1,r-1,2); M(r-1,4,r-1,5);
  ...
  // ─ Row 8: 섹션 헤더 ─
  push([hdr('인플루언서 캠페인 신청내역'),blk(),blk(),blk(),blk(),blk()], 22);
  M(r-1,0,r-1,5);

  // ─ Row 9: 테이블 헤더 ─
  push([hdr('일정'),hdr('상품명'),hdr('인플루언서명'),hdr('정액광고비(VAT별도)'),hdr('수수료율'),hdr('비고')], 22);
  ...
  camps.forEach(function(c){
    ...
    var cs      = (c.start||c.startDate||'').slice(0,10);
    var ce      = (c.end||c.endDate||'').slice(0,10);
    var feeRate = parseFloat(inf0.feeRate||c.feeRate)||0;
    push([
      cen(fmtD(cs)+(ce?'~'+fmtD(ce):'')),
      cel(c.name||''),
      cen(inf0.infName||c.infName||''),
      sd.fixedFee ? num(sd.fixedFee) : blk(),
      feeRate     ? pct(feeRate)     : blk(),
      cel('')
    ], 20);
  });
  for(var ei=camps.length; ei<EMPTY_MIN; ei++){
    push([blk(),blk(),blk(),blk(),blk(),blk()], 20);
  }
  ...
  // ─ 대행사 담당자 ─
  push([hdr('대행사 담당자'),blk(),blk(),blk(),blk(),blk()], 22); M(r-1,0,r-1,5);
  push([lbl('회사명'),  cel(mcnData.name||''),blk(),        lbl('입금 계좌번호'),cel(mcnData.bankAccount||''),blk()], 20); M(r-1,1,r-1,2); M(r-1,4,r-1,5);
  push([lbl('담당자명'),cel(mcnData.contact||''),blk(),     lbl('E-mail'),      cel(mcnData.email||''),      blk()], 20); M(r-1,1,r-1,2); M(r-1,4,r-1,5);
  push([lbl('전화번호'),cel(mcnData.phone||''),blk(),       lbl('팩스번호'),    cel(mcnData.fax||''),        blk()], 20); M(r-1,1,r-1,2); M(r-1,4,r-1,5);

  // ─ 빈 행 ─
  push([], 6);

  // ─ 특이 사항 ─
  push([hdr('특이 사항(특약)'),blk(),blk(),blk(),blk(),blk()], 22); M(r-1,0,r-1,5);
  push([cel('-'),blk(),blk(),blk(),blk(),blk()], 36); M(r-1,0,r-1,5);

  // ─ 빈 행 ─
  push([], 6);

  // ─ 계약 당사자 정보 ─
  push([hdr('계약 당사자 정보'),blk(),blk(),blk(),blk(),blk()], 22); M(r-1,0,r-1,5);
  var rPart = r;
  push([lbl('계약\n당사자'),lbl('사업자명'),cel(ssg.name),blk(),cel(mcnData.bizFullName||mcnData.name||''),blk()], 20); M(r-1,2,r-1,3); M(r-1,4,r-1,5);
  push([blk(),              lbl('주소'),    cel(ssg.addr), blk(),cel(mcnData.address||''),                 blk()], 20); M(r-1,2,r-1,3); M(r-1,4,r-1,5);
  push([blk(),              lbl('사업자번호'),cel(ssg.bizno),blk(),cel(mcnData.bizno||''),                  blk()], 20); M(r-1,2,r-1,3); M(r-1,4,r-1,5);
  push([blk(),              lbl('대표자'),  cel(ssg.ceo+' (인)'),blk(),cel((mcnData.ceo||'')+' (인)'),    blk()], 20); M(r-1,2,r-1,3); M(r-1,4,r-1,5);
  M(rPart,0,r-1,0); // 계약당사자 세로 병합

  // ─ 빈 행 ─
  push([], 6);

  // ─ 주의 사항 ─
  ['1. 본 캠페인 광고 신청서는 광고계약서를 대신하며, 집행기간동안 효력이 발생합니다.',
   '2. 본 캠페인 광고 신청서를 2부 작성, 기명날인 후 각각 1부씩 보관합니다.',
   '3. 신규 광고주의 경우, 광고주 / 광고대행사 사업자 등록증 사본 각 1부를 첨부하여 주십시오.'
  ].forEach(function(note){
    push([{v:note,t:'s',s:{font:{name:'맑은 고딕',sz:8},alignment:{horizontal:'left',vertical:'center'}}},blk(),blk(),blk(),blk(),blk()], 16);
    M(r-1,0,r-1,5);
  });
  ...
  ws['!merges'] = merges;
  ws['!rows']   = rows;
  ws['!cols']   = [{wch:12},{wch:26},{wch:16},{wch:20},{wch:10},{wch:20}];
  return ws;
}
...
function exportS7ExcelFiltered(camps){
  ...
  var hdrs = ['캠페인 시작일','캠페인 종료일','입금예정일','캠페인코드(KEY)','캠페인명','상품코드','MCN업체','인플루언서','당사유입수','구매자수','구매전환율(%)','신규회원수','공구가(원)','순주문수','순주문 금액(정산기준)','광고수수료(%)','수수료 광고비(부가세포함)','수수료 광고비2(부가세별도)','정액 광고비(부가세별도)','별도 광고비(메타광고)','세금계산서 공급가액(부가세별도)','릴스조회수','릴스댓글','정산일자','이슈 및 결과'];
  var ws = {};
  ws['!cols'] = [{wch:14},{wch:14},{wch:14},{wch:14},{wch:22},{wch:14},{wch:14},{wch:14},{wch:11},{wch:10},{wch:13},{wch:10},{wch:13},{wch:11},{wch:18},{wch:16},{wch:20},{wch:20},{wch:18},{wch:18},{wch:22},{wch:11},{wch:10},{wch:14},{wch:24}];
  var R = 0;
  hdrs.forEach(function(h,ci){ ws[XLSX.utils.encode_cell({r:R,c:ci})] = {v:h,t:'s',s:hdStyle}; });
  ...
  camps.forEach(function(c){
    ...
    // 정산일자 자동: 저장값 없으면 캠페인 마감일 + 7일
    var _campEndStr = (c.end||c.endDate||'').slice(0,10);
    var _autoSD = '';
    if(!sd.settleDate && _campEndStr){try{var _d=new Date(_campEndStr);_d.setDate(_d.getDate()+7);_autoSD=_d.toISOString().slice(0,10);}catch(e){}}
    var _mcnName = inf.mcn || c.mcn || '';
    skuItems.forEach(function(si, idx){
      R++;
      var convR = si.inflow>0 ? parseFloat((si.netOrders/si.inflow*100).toFixed(2)) : (si.convRate||0);
      var row = [
        {v:idx===0?(c.start||c.startDate||'').slice(0,10):'',t:'s',s:dtStyle},
        {v:idx===0?(c.end||c.endDate||'').slice(0,10):'',t:'s',s:dtStyle},
        {v:idx===0?(c.settlePaymentDate||''):'',t:'s',s:dtStyle},
        {v:c.campCode||'',t:'s',s:codeStyle},
        {v:c.name||'',t:'s',s:nmStyle},
        {v:si.skuCode||'',t:'s',s:dtStyle},
        {v:idx===0?_mcnName:'',t:'s',s:dtStyle},
  ...
  reader.onload = function(e){
    try{
      ...
      for(var i=0;i<Math.min(5,data.length);i++){
        // 시작일/종료일이 A열로 이동한 경우에도 감지: 행 전체에서 캠페인코드 헤더 탐색
        var found=false;
        for(var ci=0;ci<data[i].length;ci++){
          var cell=String(data[i][ci]||'').trim().replace('(KEY)','').trim();
          if(cell==='캠페인코드'){ hdrRow=i; found=true; break; }
        }
        if(found) break;
      }
